...
function DrawTimeS(rows,Business,Type){
	...
	plots
		.each(function(d,i){
				...
				var timeSeries = d3.Timeseries()
					...
					d3.select(this).datum(TimeObject(d,Type))
					.call(timeSeries)
					.append('p')
					.text(d.key);
...
{
	var TemporaryArrary = [{BusinessID:null,Values:null,States:null,Name:null,Categories:null}];
	for(var i=0;i<ReviewNest.length;i++)
	{
		...
		for(var z=0;z<Business.length;z++)
		{
			if(Business[z].BusinessId == ReviewNest[i].key)
			{
				...
				TemporaryArrary[i].Categories = Business[z].Categories;
				break;
			}
		}
		TemporaryArrary.push({BusinessID:null,Values:null,States:null,Name:null,Categories:null});
	}
...
function TimeObject(d,Type)
{
	console.log(d);
	var TemporyObject = [{Date:null,Stars:null,Categories:null}]
	...
	if(KeyWord2!="All")
		{
			for(var i=0;i<d.values.length;i++)
			{
				...
				for(var m=0;m<d.values[i].Categories.length;m++)
					{
						var JudgeString = CheckCategoryToGroup(d.values[i].Categories[m],Type);
						console.log(JudgeString);
						if(JudgeString == KeyWord2)
							{
								for(var z=0;z<d.values[i].Values.length;z++)
									{
										...
										TemporyObject[TemporyObject.length-1].Categories = d.values[i].Categories;
										TemporyObject.push({Date:null,Stars:null,Categories:null});
									}
								break;
							}
					}
